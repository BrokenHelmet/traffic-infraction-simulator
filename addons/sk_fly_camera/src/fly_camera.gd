...
## [br][br]
##
## The camera controls can be changed. By default it uses:
## - [param WASD] keys for movement
## - [param E/Q] keys for vertical movement (up/down on global Y axis)
## - [param Shift] to move faster
## - [param Ctrl] to move slower
## - [param Right Mouse Button] to activate/deactivate mouse controls
## [br][br]
##
...
func _unhandled_input(event: InputEvent) -> void:
	# Handle mouse capture activation/deactivation
...
func _physics_process(delta: float) -> void:
	...
	if _mouse_hidden or keep_key_controls:
		...
		if _is_cam_action_pressed(_ACTION_UP):       dir += Vector3.UP
		if _is_cam_action_pressed(_ACTION_DOWN):     dir += Vector3.DOWN
...
func _check_mouse_capture(event:InputEvent) -> void:
	"""Handle mouse button events for camera activation/deactivation"""
	var button_state := 0    # 0- unchanged, 1- pressed, 2- released

	# Check for mouse button events or custom input actions
